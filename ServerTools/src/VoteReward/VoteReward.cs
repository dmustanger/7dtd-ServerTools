...
namespace ServerTools
{
    class VoteReward
    {
        public static bool IsEnabled = false, IsRunning = false, RandomListRunning = true, Reward_Entity = false;
        public static int Reward_Count = 1, Delay_Between_Uses = 24, Entity_Id = 74, _CountItems = 0;
        ...
        private static SortedDictionary<string, int[]> dict1 = new SortedDictionary<string, int[]>();
        private static SortedDictionary<int, int> rewardCount = new SortedDictionary<int, int>();
        private static FileSystemWatcher fileWatcher = new FileSystemWatcher(API.ConfigPath, file);
        private static bool updateConfig = false, posFound = false;
        public static System.Random rnd = new System.Random();

        private static void BuildDict1()
        {
            if (dict.Count > 0)
            {
        ...
        public static void LoadXml()
        {
            ...
            foreach (XmlNode childNode in _XmlNode.ChildNodes)
            {
                if (childNode.Name == "Rewards")
                {
                    ...
                    int _dictCount = dict.Count;
                    if (Reward_Count > _dictCount)
                    {
                        Reward_Count = _dictCount;
                    }
                    BuildDict1();
                }
            }
        ...
        private static void ItemOrBlock(ClientInfo _cInfo)
        {
            if (Reward_Count > 0)
            {
                ...
                if (dict1.Count > 0)
                {
                    int _rndItem = rnd.Next(1, dict1.Count + 1);
                    _item = dict1.ElementAt(_rndItem).Key;
                }
                else
                {
                    BuildDict1();
                ...
                int[] _values;
                if (dict1.TryGetValue(_item, out _values))
                {
                    int count = 0;
                    ...
                    if (count > 0)
                    {
                        ...
                        if (rewardCount.ContainsKey(_cInfo.entityId))
                        {
                            int _count;
                            if (rewardCount.TryGetValue(_cInfo.entityId, out _count))
                            {
                                int _count1 = _count + 1;
                                if (_count1 != Reward_Count)
                                {
                                    rewardCount.Remove(_cInfo.entityId);
                                    rewardCount.Add(_cInfo.entityId, _count1);
                                    dict1.Remove(_item);
                                    ItemOrBlock(_cInfo);
                                }
                                else
                        ...
                        {
                            rewardCount.Add(_cInfo.entityId, 1);
                            ItemOrBlock(_cInfo);
                        }
                    }
                    else
        ...
        private static void Entity(ClientInfo _cInfo)
        {
            ...
            if (posFound)
            {
                ...
                if (eClass.bAllowUserInstantiate)
                {
                    ...
                    Log.Out(string.Format("Spawned an entity reward {0} at {1} x, {2} y, {3} z for {4}", eClass.entityClassName, _x, _y, _z, _cInfo.playerName));
                }
            }
            else
            {
                Log.Out(string.Format("No spawn points were found near {0} x, {1} y, {2} z. {3} did not receive their reward.", _x, _y, _z, _cInfo.playerName));
            }
        }
    }
}
