...
namespace ServerTools
{
    public class Config
    {
        public const string Version = "20.3.1";
            ...
            if (childNodes != null)
            {
                for (int i = 0; i < childNodes.Count; i++)
                {
                    if (childNodes[i].NodeType != XmlNodeType.Comment)
                    {
                        ...
                        if (childNodes[i].Name == "AntiCheat" || childNodes[i].Name == "Tools")
                        {
                            for (int j = 0; j < childNodes[i].ChildNodes.Count; j++)
                            {
                                XmlElement line = (XmlElement)childNodes[i].ChildNodes[j];
                                    ...
                                    if (line.HasAttribute("Name"))
                                    {
                                        switch (line.GetAttribute("Name"))
                                        {
                                            ...
                                            case "Auction":
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Auction entry in ServerToolsConfig.xml because of missing 'Total_Items' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Total_Items"), out Auction.Total_Items))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Auction entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Total_Items' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                            ...
                                            case "Auto_Backup":
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Auto_Backup entry in ServerToolsConfig.xml because of missing 'Destination' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (line.HasAttribute("Destination"))
                                                {
                                            ...
                                            case "Damage_Detector":
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Damage_Detector entry in ServerToolsConfig.xml because of missing 'Entity_Damage_Limit' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Entity_Damage_Limit"), out DamageDetector.Entity_Damage_Limit))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Damage_Detector entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Entity_Damage_Limit' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Block_Damage_Limit"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Damage_Detector entry in ServerToolsConfig.xml because of missing 'Block_Damage_Limit' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Block_Damage_Limit"), out DamageDetector.Block_Damage_Limit))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Damage_Detector entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Block_Damage_Limit' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Player_Damage_Limit"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Damage_Detector entry in ServerToolsConfig.xml because of missing 'Player_Damage_Limit' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Player_Damage_Limit"), out DamageDetector.Player_Damage_Limit))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Damage_Detector entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Player_Damage_Limit' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Admin_Level"))
                                                {
                                            ...
                                            case "Discord_Link":
                                                ...
                                                if (!line.HasAttribute("Invitation_Link"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Discord_Link entry in ServerToolsConfig.xml because of missing 'Invitation_Link' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (line.HasAttribute("Invitation_Link"))
                                                {
                                                    string link = line.GetAttribute("Invitation_Link");
                                                    if (DiscordLink.Invitation_Link != link)
                                                    {
                                                        DiscordLink.Invitation_Link = link;
                                            ...
                                            case "FPS":
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring FPS entry in ServerToolsConfig.xml because of missing 'Set_Target' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Set_Target"), out Fps.Set_Target))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring FPS entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Set_Target' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Low_FPS"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring FPS entry in ServerToolsConfig.xml because of missing 'Low_FPS' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Low_FPS"), out Fps.Low_FPS))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring FPS entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Low_FPS' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                break;
                                            ...
                                            case "Invalid_Items":
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Invalid_Items entry in ServerToolsConfig.xml because of missing 'Check_Storage' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Check_Storage"), out InvalidItems.Check_Storage))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Invalid_Items entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Check_Storage' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                break;
                                            ...
                                            case "Kill_Notice":
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Kill_Notice entry in ServerToolsConfig.xml because of missing 'PvP' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                            ...
                                            case "Lobby_Extended":
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of missing 'Reserved_Only' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Reserved_Only"), out Lobby.Reserved_Only))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Reserved_Only' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Command_Cost"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of missing 'Command_Cost' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Command_Cost"), out Lobby.Command_Cost))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Command_Cost' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Player_Check"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of missing 'Player_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Player_Check"), out Lobby.Player_Check))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Player_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Zombie_Check"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of missing 'Zombie_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Zombie_Check"), out Lobby.Zombie_Check))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Zombie_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("PvE"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of missing 'PvE' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("PvE"), out Lobby.PvE))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lobby entry in ServerToolsConfig.xml because of invalid (True/False) value for 'PvE' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                break;
                                            case "Location":
                                            ...
                                            case "Lottery":
                                                ...
                                                if (!line.HasAttribute("Bonus"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lottery entry in ServerToolsConfig.xml because of missing 'Bonus' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Bonus"), out Lottery.Bonus))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Lottery entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Bonus' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                break;
                                            case "Market":
                                            ...
                                            case "Market_Extended":
                                                if (!line.HasAttribute("Reserved_Only"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of missing 'Reserved_Only' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Reserved_Only"), out Market.Reserved_Only))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Reserved_Only' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Command_Cost"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of missing 'Command_Cost' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!int.TryParse(line.GetAttribute("Command_Cost"), out Market.Command_Cost))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Command_Cost' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Player_Check"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of missing 'Player_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Player_Check"), out Market.Player_Check))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Player_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Zombie_Check"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of missing 'Zombie_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Zombie_Check"), out Market.Zombie_Check))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Zombie_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("PvE"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of missing 'PvE' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("PvE"), out Market.PvE))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Market entry in ServerToolsConfig.xml because of invalid (True/False) value for 'PvE' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                break;
                                            case "Message_Color":
                                            ...
                                            case "Net_Package_Detector":
                                                if (!line.HasAttribute("Enable"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Net_Package_Detector entry in ServerToolsConfig.xml because of missing 'Enable' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Enable"), out PersistentOperations.Net_Package_Detector))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Net_Package_Detector entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Enable' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                break;
                                            case "New_Player":
                                            ...
                                            case "Tracking":
                                                if (!line.HasAttribute("Enable"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Tracking entry in ServerToolsConfig.xml because of missing 'Enable' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Enable"), out Track.IsEnabled))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Tracking entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Enable' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                break;
                                            case "Travel":
                                            ...
                                            case "Vehicle_Recall":
                                                ...
                                                if (!line.HasAttribute("Inside_Claim"))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Vehicle_Recall entry in ServerToolsConfig.xml because of missing 'Inside_Claim' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!bool.TryParse(line.GetAttribute("Inside_Claim"), out VehicleRecall.Inside_Claim))
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Vehicle_Recall entry in ServerToolsConfig.xml because of invalid (True/False) value for 'Inside_Claim' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                            ...
                                            case "Voting":
                                                ...
                                                if (line.HasAttribute("Your_Voting_Site"))
                                                {
                                                    string voteSite = line.GetAttribute("Your_Voting_Site");
                                                    if (VoteReward.Your_Voting_Site != voteSite)
                                                    {
                                                        VoteReward.Your_Voting_Site = voteSite;
                                                        VoteReward.SetLink(voteSite);
                                                    }
                                                }
                                            ...
                                            case "Waypoints_Extended":
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Waypoints_Extended entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Player_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Zombie_Check"))
                                                {
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Waypoints_Extended entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Zombie_Check' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                if (!line.HasAttribute("Vehicle"))
                                                {
                                                ...
                                                {
                                                    Log.Warning(string.Format("[SERVERTOOLS] Ignoring Waypoints_Extended entry in ServerToolsConfig.xml because of invalid (non-numeric) value for 'Vehicle' attribute: {0}", line.OuterXml));
                                                    continue;
                                                }
                                                break;
                                            case "Web_API":
        ...
        public static void WriteXml()
        {
            ...
            using (StreamWriter sw = new StreamWriter(ConfigFilePath, false, Encoding.UTF8))
            {
                ...
                sw.WriteLine(string.Format("        <Tool Name=\"Discord_Link\" Enable=\"{0}\" Invitation_Link=\"{1}\" />", DiscordLink.IsEnabled, DiscordLink.Invitation_Link));
                ...
                sw.WriteLine(string.Format("        <Tool Name=\"Entity_Cleanup\" Enable=\"{0}\" Falling_Tree=\"{1}\" Underground=\"{2}\" Delete_Bicycles=\"{3}\" Delete_Drones=\"{4}\" />", EntityCleanup.IsEnabled, EntityCleanup.FallingTreeEnabled, EntityCleanup.Underground, EntityCleanup.Bicycles, EntityCleanup.Drones));
