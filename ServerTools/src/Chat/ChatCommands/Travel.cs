...
namespace ServerTools
{
    class Travel
    {
        ...
        public static void CheckAndTele(ClientInfo _cInfo, bool _announce, string _playerName)
        {
            var enumerator = GameManager.Instance.World.Players.list;
                            ...
                            foreach (KeyValuePair<string, string> kvpXYZmax in XYZmax)
                            {
                                foreach (KeyValuePair<string, string> destination in Destination)
                                {
                                    if (kvpXYZmin.Key == kvpXYZmax.Key & kvpXYZmin.Key == destination.Key)
                                    {
                                        ...
                                        {
                                            SdtdConsole.Instance.ExecuteSync(string.Format("tele {0} {1} {2} {3}", _cInfo.entityId, _xDest, _yDest, _zDest), _cInfo);
                                            string _phrase603;
