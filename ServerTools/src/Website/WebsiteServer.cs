...
{
    public partial class WebsiteServer
    {
        ...
        private static void BuildLists()
        {
            try
            {
                List<string> _clientList = PersistentContainer.Instance.WebsiteClientList;
            ...
            {
                Log.Out(string.Format("[SERVERTOOLS] Error in WebsiteServer.BuildLists: {0}" + e.Message));
            }
        }
                ...
                if (_host4 > 0 && _host4 < 65536)
                {
                    if (HttpListener.IsSupported)
                    {
                        ...
                        if (string.IsNullOrEmpty(_externalIp))
                        {
                            ...
                            if (string.IsNullOrEmpty(_externalIp))
                            {
                                Log.Out("[SERVERTOOLS] This host can not support ServerTools website operator. It has been disabled");
                                return;
                            }
                        }
                        ...
                        Log.Out(string.Format("[SERVERTOOLS] ServerTools website has opened on port {0}", _host4.ToString()));
                    ...
                    {
                        Log.Out(string.Format("[SERVERTOOLS] This host can not support ServerTools website operator. It has been disabled"));
                    }
                }
                else
                {
                    Log.Out(string.Format("[SERVERTOOLS] Control panel port is set too high. It must be between 1-65531 for ServerTools website to function. Website has been disabled"));
                }
            }
            catch (Exception e)
            {
                Writer(string.Format("Error in WebsiteServer.Exec: {0}", e.Message));
                Log.Out(string.Format("[SERVERTOOLS] Error in WebsiteServer.Exec: {0}" + e.Message));
                Listener.Stop();
                Listener.Close();
                Unload();
                Load();
                Writer(string.Format("Restarted website server automatically"));
            }
        }
        ...
        private static void IsAllowed(HttpListenerRequest _request, HttpListenerResponse _response, string _clientIp)
        {
            ...
            {
                Writer(string.Format("Error in WebsiteServer.IsAllowed: {0}", e.Message));
            }
            _response.Close();
        }
            ...
            try
            {
                ...
                if (_fileInfo != null && _fileInfo.Exists)
                {
                    ...
                    if (_c != null)
                    {
                        ...
                        Writer(string.Format("Website request: {0} from IP {1}", SITE_DIR + _uri, _request.RemoteEndPoint.Address.ToString()));
                    }
                    else
                    {
                ...
                {
                    _response.StatusCode = 404;
                    Writer(string.Format("Requested website file not found at {0}", SITE_DIR + _uri));
                }
            }
            catch (Exception e)
            {
                Writer(string.Format("Error in WebsiteServer.GET: {0}", e.Message));
            }
            _response.Close();
        }
        ...
        private static void POST(HttpListenerRequest _request, HttpListenerResponse _response, string _uri, string _postMessage)
        {
            try
            {
                if (_postMessage.Length == 4000)
                {
                                ...
                                if (_decryptSplit[0] == _clientHandle[1])
                                {
                                    if (!string.IsNullOrEmpty(_decryptSplit[1]) && Clients.Contains(_decryptSplit[1]))
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(_decryptSplit[2]) && !string.IsNullOrEmpty(PersistentContainer.Instance.Players[_decryptSplit[1]].WP) && _decryptSplit[2] == PersistentContainer.Instance.Players[_decryptSplit[1]].WP)
                                        {
                                            ...
                                            _response.StatusCode = 200;
                                            Writer(string.Format("Website client {0} has signed in from IP {1}", _decryptSplit[1], _request.RemoteEndPoint.Address.ToString()));
                                        }
                                        else
                                        {
                                            if (LoginAttempts.ContainsKey(_ip))
                                            ...
                                            _response.StatusCode = 401;
                                            Writer(string.Format("Website sign in failure by IP {0}", _request.RemoteEndPoint.Address.ToString()));
                                        }
                                    }
                                    else
                                    {
                        ...
                        if (ValidAuth(_id))
                        {
                            ...
                            if (_decrypt.Contains(' '))
                            {
                                ...
                                if (_decryptSplit[0] == _clientHandle[1])
                                {
                                    ...
                                    _response.StatusCode = 200;
                                    Writer(string.Format("Website client has signed out from IP {0}", _request.RemoteEndPoint.Address.ToString()));
                                }
                                else
                                {
                                    _response.StatusCode = 401;
                    ...
                    //
                    //                        SdtdConsole.Instance.Output(string.Format("[SERVERTOOLS] Website console command: {0}. Run by client {1} with IP {2}", _decryptSplit[2], _decryptSplit[1], _request.RemoteEndPoint.Address.ToString()));
                    //                        _responseMessage += SetKey(_id, _clientHandle);
                    //                        _response.StatusCode = 200;
                    //                    }
                    //                }
                    ...
                    else if (_uri == "Config")
                    {
                        ...
                        {
                            FollowUp.Remove(_id);
                            if (Utils.FileExists(LoadConfig.configFilePath))
                            {
                                XmlDocument xmlDoc = new XmlDocument();
                                try
                                {
                                    xmlDoc.Load(LoadConfig.configFilePath);
                                }
                                catch (XmlException e)
                                {
                                    Log.Out(string.Format("[SERVERTOOLS] Failed loading {0}: {1}", LoadConfig.configFilePath, e.Message));
                                    return;
                                }
                    ...
                    else if (_uri == "UpdateConfig")
                    {
                        ...
                        else
                        {
                            ...
                            if (_newConfig.Contains("☼") && _newConfig.Contains("§") && Utils.FileExists(LoadConfig.configFilePath))
                            {
                                int _count = 0;
                                ...
                                try
                                {
                                    _xmlDoc.Load(LoadConfig.configFilePath);
                                }
                                catch (XmlException e)
                                {
                                    Log.Out(string.Format("[SERVERTOOLS] Failed loading {0}: {1}", LoadConfig.configFilePath, e.Message));
                                ...
                                _xmlDoc.Save(LoadConfig.configFilePath);
                            }
                        }
                    }
                    else if (_uri == "Kick")
                                    ...
                                    if (!string.IsNullOrEmpty(_decryptSplit[1]) && Clients.Contains(_decryptSplit[1]))
                                    {
                                        if (!string.IsNullOrEmpty(_decryptSplit[2]))
                                        {
                                            ...
                                            if (_cInfo != null)
                                            {
                                                ...
                                                Writer(string.Format("Website client {0} has kicked {1} from IP {2}", _decryptSplit[1], _decryptSplit[2], _request.RemoteEndPoint.Address.ToString()));
                                            }
                                            else
                                            {
                                    ...
                                    if (!string.IsNullOrEmpty(_decryptSplit[1]) && Clients.Contains(_decryptSplit[1]))
                                    {
                                        if (!string.IsNullOrEmpty(_decryptSplit[2]))
                                        {
                                            ...
                                            if (_cInfo != null)
                                            {
                                                ...
                                                Writer(string.Format("Website client {0} has banned {1} from IP {2}", _decryptSplit[1], _decryptSplit[2], _request.RemoteEndPoint.Address.ToString()));
                                            }
                                            else
                                            {
                                                _response.StatusCode = 202;
                                            ...
                                            if (_cInfo != null)
                                            {
                                                ...
                                                if (Jail.Jailed.Contains(_cInfo.playerId))
                                                {
                                                    ...
                                                    if (_player != null)
                                                    {
                                                        ...
                                                        Writer(string.Format("Website client {0} has unjailed {1} from IP {2}", _decryptSplit[1], _decryptSplit[2], _request.RemoteEndPoint.Address.ToString()));
                                                    }
                                                }
                                    ...
                                    if (!string.IsNullOrEmpty(_decryptSplit[1]) && Clients.Contains(_decryptSplit[1]))
                                    {
                                        if (!string.IsNullOrEmpty(_decryptSplit[2]))
                                        {
                                            ...
                                            if (_cInfo != null)
                                            {
                                                ...
                                                Writer(string.Format("Website client {0} has rewarded {1} from IP {2}", _decryptSplit[1], _decryptSplit[2], _request.RemoteEndPoint.Address.ToString()));
                                            }
                                            else
                                            {
                                                _response.StatusCode = 202;
                ...
                else
                {
                    ...
                    Writer("Client with IP {0} has been banned from the website. Detected improper message format sent to server");
                }
            }
            catch (Exception e)
            {
