...
{
    class PlayerStatsPackage
    {

        private static AccessTools.FieldRef<NetPackagePlayerStats, int> entityId = AccessTools.FieldRefAccess<NetPackagePlayerStats, int>("entityId");
        ...
        public static bool IsValid (NetPackagePlayerStats __instance)
        {
            try
            {
                ...
                {
                    ClientInfo cInfo = GeneralFunction.GetClientInfoFromEntityId(entityId(__instance));
                    if (cInfo != null)
                    {
