                ...
                foreach (var _player in enumerator)
                {
                    ...
                    if (LastPosition.ContainsKey(_player.entityId))
                    {
                        ...
                        if (ZoneWarnings & !ZoneEdge.Contains(_player.entityId))
                        {
                            ...
                            {
                                _cInfo.SendPackage(new NetPackageGameMessage(EnumGameMessages.Chat, string.Format("{0}You are near the edge of the world {1}[-]", Config.ChatColor, _cInfo.playerName), "Server", false, "", false));
