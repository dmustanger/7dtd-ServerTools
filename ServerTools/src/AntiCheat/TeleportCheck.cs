    ...
    class TeleportCheck
    {
        ...
        public static void Penalty(ClientInfo _cInfo)
        {
            if (Jail_Enabled)
            {
                if (Announce)
                {
                    GameManager.Instance.GameMessageServer((ClientInfo)null, EnumGameMessages.Chat, string.Format("{0}{1} has been jailed for teleporting.[-]", Config.Chat_Response_Color, _cInfo.playerName), "Server", false, "", false);
                }
            ...
            if (Kill_Enabled)
            {
                if (Announce)
                {
                    GameManager.Instance.GameMessageServer((ClientInfo)null, EnumGameMessages.Chat, string.Format("{0}{1} has been executed for teleporting.[-]", Config.Chat_Response_Color, _cInfo.playerName), "Server", false, "", false);
                }
            ...
            if (Kick_Enabled)
            {
                if (Announce)
                {
                    GameManager.Instance.GameMessageServer((ClientInfo)null, EnumGameMessages.Chat, string.Format("{0}{1} has been kicked for teleporting.[-]", Config.Chat_Response_Color, _cInfo.playerName), "Server", false, "", false);
                }
            ...
            if (Ban_Enabled)
            {
                if (Announce)
                {
                    GameManager.Instance.GameMessageServer((ClientInfo)null, EnumGameMessages.Chat, string.Format("{0}{1} has been banned for teleporting.[-]", Config.Chat_Response_Color, _cInfo.playerName), "Server", false, "", false);
                }
