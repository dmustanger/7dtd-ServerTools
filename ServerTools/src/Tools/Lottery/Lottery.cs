    ...
    class Lottery
    {
        ...
        public static void Response(ClientInfo _cInfo)
        {
            if (OpenLotto)
            {
                ...
                _phrase222 = _phrase222.Replace("{CommandPrivate}", ChatHook.Command_Private);
                _phrase222 = _phrase222.Replace("{Command85}", Command85);
                ChatHook.ChatMessage(_cInfo, LoadConfig.Chat_Response_Color + _phrase222 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Whisper, null);
            }
            else
            {
                Phrases.Dict.TryGetValue(221, out string _phrase221);
                _phrase221 = _phrase221.Replace("{CommandPrivate}", ChatHook.Command_Private);
                _phrase221 = _phrase221.Replace("{Command84}", Command84);
                ChatHook.ChatMessage(_cInfo, LoadConfig.Chat_Response_Color + _phrase221 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Whisper, null);
            }
        }
                    ...
                    if (int.TryParse(_message, out _lottoValue))
                    {
                        if (_lottoValue > 0)
                        {
                            if (Wallet.GetCurrentCoins(_cInfo.playerId) >= _lottoValue)
                            {
                                ...
                                ChatHook.ChatMessage(_cInfo, LoadConfig.Chat_Response_Color + _phrase224 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Whisper, null);
                                ...
                                _phrase225 = _phrase225.Replace("{CommandPrivate}", ChatHook.Command_Private);
                                _phrase225 = _phrase225.Replace("{Command85}", Command85);
                                ChatHook.ChatMessage(null, LoadConfig.Chat_Response_Color + _phrase225 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Global, null);
                            }
                            else
                            {
                                Phrases.Dict.TryGetValue(226, out string _phrase226);
                                _phrase226 = _phrase226.Replace("{CoinName}", Wallet.Coin_Name);
                                ChatHook.ChatMessage(_cInfo, LoadConfig.Chat_Response_Color + _phrase226 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Whisper, null);
                            }
                        }
                        else
                        {
                            Phrases.Dict.TryGetValue(223, out string _phrase223);
                            ChatHook.ChatMessage(_cInfo, LoadConfig.Chat_Response_Color + _phrase223 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Whisper, null);
                        }
                    }
                    else
                    {
            ...
            if (OpenLotto)
            {
                if (Wallet.GetCurrentCoins(_cInfo.playerId) >= LottoValue)
                {
                    if (!LottoEntries.Contains(_cInfo))
                    {
                        ...
                        ChatHook.ChatMessage(_cInfo, LoadConfig.Chat_Response_Color + _phrase227 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Whisper, null);
                    ...
                    else
                    {
                        ...
                        ChatHook.ChatMessage(_cInfo, LoadConfig.Chat_Response_Color + _phrase228 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Whisper, null);
                    }
                }
                else
                {
        ...
        public static void StartLotto()
        {
            ...
            ChatHook.ChatMessage(null, LoadConfig.Chat_Response_Color + _phrase230 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Global, null);
        }

        public static void Alert()
        {
            Phrases.Dict.TryGetValue(229, out string _phrase229);
            _phrase229 = _phrase229.Replace("{CommandPrivate}", ChatHook.Command_Private);
            _phrase229 = _phrase229.Replace("{Command85}", Command85);
            ChatHook.ChatMessage(null, LoadConfig.Chat_Response_Color + _phrase229 + "[-]", -1, LoadConfig.Server_Response_Name, EChatType.Global, null);
        }
    }
}
