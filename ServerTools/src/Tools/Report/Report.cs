        ...
        public static void Exec(ClientInfo _cInfo, string _message)
        {
            ...
            if (clientList != null)
            {
                for (int i = 0; i < clientList.Count; i++)
                {
                    ...
                    if (GameManager.Instance.adminTools.Users.GetUserPermissionLevel(cInfoAdmin.PlatformId) <= Admin_Level &&
                        GameManager.Instance.adminTools.Users.GetUserPermissionLevel(cInfoAdmin.CrossplatformId) <= Admin_Level)
                    {
