        ...
        public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
        {
            try
            {
                ...
                if (_params[0].ToLower() == "new")
                {
                    ...
                    {
                        SdtdConsole.Instance.Output("[SERVERTOOLS] Unable to find a valid id to attach as the event operator. New events must be setup while in game");
                        return;
                    }
                    _params.RemoveAt(0);
                    ...
                    if (Event.Open)
                    {
                        ...
                        foreach (var _player in Event.Teams)
                        {
                            ...
                            if (cInfo != null)
                            {
                                ...
                                SdtdConsole.Instance.Output(string.Format("Online player: Name '{0}' Id '{1}' is on team '{2}'.", cInfo.playerName, cInfo.CrossplatformId.CombinedString, _team));
                            }
                            else
                            {
                                Event.Teams.TryGetValue(_player.Key, out int _team);
                                PlayerDataFile _pdf = GeneralOperations.GetPlayerDataFileFromId(_player.Key);
                                if (_pdf != null)
                                {
