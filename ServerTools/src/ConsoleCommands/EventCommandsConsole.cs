        ...
        public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
        {
            try
            {
                ...
                if (_params[0].ToLower() == ("new"))
                {
                    ...
                    string _buildName = _params[1];
                    for (int i = 2; i < _params.Count; i++)
                    {
                        _buildName = _buildName + " " + _params[i];
                    }
                    string _name = SQL.EscapeString(_buildName);
                    string _sql1 = string.Format("SELECT eventName FROM Events WHERE eventAdmin = '{0}' AND eventName = '{1}'", _steamId, _buildName);
                    DataTable _result = SQL.TQuery(_sql1);
                    if (_result.Rows.Count == 0)
                    {
                        _result.Dispose();
                        string _sql2 = string.Format("INSERT INTO Events (eventAdmin, eventName) VALUES ('{0}', '{1}')", _steamId, _buildName);
                        SQL.FastQuery(_sql2);
                        if (Event.SetupStage.ContainsKey(_entityId))
                        {
                            Event.SetupStage[_entityId] = 1;
                        }
                        else
                        {
                            Event.SetupStage.Add(_entityId, 1);
                        }
                        SdtdConsole.Instance.Output("You have started to open a new event.");
                        SdtdConsole.Instance.Output(string.Format("The event name has been set to {0}.", _buildName));
                    ...
                    {
                        _result.Dispose();
                        SdtdConsole.Instance.Output(string.Format("The event name {0} already exists. Use a new name or delete the old event.", _buildName));
                        return;
                    }
                }
                else if (_params[0].ToLower() == ("save"))
                {
                    ...
                    {
                        int _stage;
                        if (Event.SetupStage.TryGetValue(_entityId, out _stage))
                        {
                            string _eventName;
                            Event.SetupName.TryGetValue(_entityId, out _eventName);
                            if (_stage == 3)
                            {
                                ...
                                {
                                    _result.Dispose();
                                    string _sql2 = string.Format("INSERT INTO EventSpawns (eventid, eventTeam, eventSpawn) VALUES ({0}, {1}, '{2}')", _eventid, _team, _sposition);
                                    SQL.FastQuery(_sql2);
                                    Event.SetupStage[_entityId] = 4;
                            ...
                            else if (_stage == 4)
                            {
                                ...
                                {
                                    _result.Dispose();
                                    string _sql2 = string.Format("INSERT INTO EventSpawns (eventid, eventTeam, eventRespawn) VALUES ({0}, {1}, '{2}')", _eventid, _team, _sposition);
                                    SQL.FastQuery(_sql2);
                                    Event.SetupStage[_entityId] = 5;
                ...
                else
                {
                    if (Event.SetupStage.ContainsKey(_entityId))
                    {
                        ...
                        if (Event.SetupStage.TryGetValue(_entityId, out _stage))
                        {
                            ...
                            if (_stage == 1)
                            {
                                string _buildInvite = _params[1];
                                for (int i = 2; i < _params.Count; i++)
                                {
                                    _buildInvite = _buildInvite + " " + _params[i];
                                }
                                string _invite = SQL.EscapeString(_buildInvite);
                                string _sql = string.Format("UPDATE Events SET eventInvite = '{0}' WHERE eventAdmin = '{1}' AND eventName = '{2}'", _invite, _steamId, _eventName);
                                SQL.FastQuery(_sql);
                                Event.SetupStage[_entityId] = 2;
                                SdtdConsole.Instance.Output(string.Format("The event invitation has been set to {0}.", _buildInvite));
                ...
                if (_params[0] == ("check"))
                {
                    if (Event.Open)
                    {
                        if (Event.Admin == _steamId)
                        {
                            ...
                            for (int i = 0; i < Event.PlayersTeam.Count; i++)
                            {
                                int _playerEntId = Event.PlayersTeam[i];
                                ...
                                for (int i = 0; i < Event.PlayersTeam.Count; i++)
                                {
                                    ...
                                    if (_cInfo != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            Event.SpawnList.Remove(_playerEntId);
                                            Event.PlayersTeam.Remove(_playerEntId);
                                            SdtdConsole.Instance.Output(string.Format("Player with Id {0} was not spawned but they have been removed from the event and set to go back to their return point.", _playerEntId));
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        Event.SpawnList.Remove(_playerEntId);
                                        Event.PlayersTeam.Remove(_playerEntId);
                                        SdtdConsole.Instance.Output(string.Format("Player with Id {0} was offline but they have been removed from the event and set to go back to their return point.", _playerEntId));
                                    }
                                }
                ...
                if (_params[0] == ("remove"))
                {
                    if (Event.Admin == _steamId)
                    {
                        if (Event.Open)
                        {
                            ...
                            {
                                if (Event.PlayersTeam.ContainsKey(_id))
                                {
                                    ...
                                    {
                                        string _sql1 = string.Format("UPDATE Players SET return = 'true' WHERE steamid = '{0}'", _id);
                                        SQL.FastQuery(_sql1);
                                        Event.SpawnList.Remove(_id);
                                        Event.PlayersTeam.Remove(_id);
