        ...
        public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
        {
            try
            {
                ...
                if (_params.Count == 2)
                {
                    ...
                    {
                        SdtdConsole.Instance.Output(string.Format("Invalid backpack id value: {0}", _entityId1));
                        return;
                    }
                    int _entityId2;
                    if (!int.TryParse(_params[1], out _entityId2))
                    {
                        SdtdConsole.Instance.Output(string.Format("Invalid backpack id value: {0}", _entityId2));
                        return;
                    }
                    Entity Entity1 = GameManager.Instance.World.Entities.dict[_entityId1];
                    if (Entity1 == null)
                    {
                        SdtdConsole.Instance.Output(string.Format("Backpack not found: {0}", _entityId1));
                        return;
                    }
                    if (Entity1.IsClientControlled())
                    {
                        SdtdConsole.Instance.Output(string.Format("Backpack with id {0} can not be a player. Use a different command for teleporting players", _entityId1));
                        return;
                    }
                            ...
                            if (Entity1.belongsPlayerId != 0)
                            {
                                ...
                                foreach (KeyValuePair<int, EntityClass> i in EntityClass.list.Dict)
                                {
                                    if (i.Value == Entity1.EntityClass)
                                    {
                                        ...
                                        SdtdConsole.Instance.Output(string.Format("Spawned a new backpack to {0} {1} {2} with the old backpack loot", Entity2.position.x, Entity2.position.y, Entity2.position.z));
                                        return;
                                    }
                                }
                            }
                ...
                if (_params.Count == 4)
                {
                    ...
                    {
                        SdtdConsole.Instance.Output(string.Format("Invalid backpack id value: {0}", _entityId));
                        return;
                    }
                    Entity Entity = GameManager.Instance.World.Entities.dict[_entityId];
                    if (Entity == null)
                    {
                        SdtdConsole.Instance.Output(string.Format("Invalid backpack. Backpack not found: {0}", _entityId));
                        return;
                    }
                    if (Entity.IsClientControlled())
                    {
                        SdtdConsole.Instance.Output(string.Format("Backpack with id {0} can not be a player. Use a different command for teleporting players", _entityId));
                        return;
                    }
                    float _x, _y, _z;
                    if (!float.TryParse(_params[1], out _x))
                    {
                        SdtdConsole.Instance.Output(string.Format("Invalid backpack value: {0}", _x));
                        return;
                    }
                    if (!float.TryParse(_params[2], out _y))
                    {
                        SdtdConsole.Instance.Output(string.Format("Invalid backpack value: {0}", _y));
                        return;
                    }
                    if (!float.TryParse(_params[3], out _z))
                    {
                        SdtdConsole.Instance.Output(string.Format("Invalid backpack value: {0}", _z));
                        return;
                    }
                            ...
                            if (Entity.belongsPlayerId != 0)
                            {
                                ...
                                foreach (KeyValuePair<int, EntityClass> i in EntityClass.list.Dict)
                                {
                                    if (i.Value == Entity.EntityClass)
                                    {
                                        ...
                                        entity.lootContainer.items = _items;
                                        GameManager.Instance.World.SpawnEntityInWorld(entity);
                                        SdtdConsole.Instance.Output(string.Format("Spawned a new backpack to {0} {1} {2} with the old backpack loot", _x, _y, _z));
                                        return;
                                    }
                                }
                            }
            ...
            {
                Log.Out(string.Format("[SERVERTOOLS] Error in TeleportBackpackConsole.Execute: {0}", e));
            }
        }
    }
}
