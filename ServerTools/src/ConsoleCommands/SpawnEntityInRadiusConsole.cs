    ...
    public class SpawnEntityInRadiusConsole : ConsoleCmdAbstract
    {
        ...
        public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
        {
            try
            {
                ...
                {
                    SdtdConsole.Instance.Output(string.Format("[SERVERTOOLS] Wrong number of arguments, expected 3 to 5, found {0}", _params.Count));
                    return;
                }
                ...
                if (_params.Count == 3)
                {
                    ...
                    if (GameManager.Instance.World.Players.dict.ContainsKey(_cInfo.entityId))
                    {
                        ...
                        if (float.TryParse(_params[1], out _radius))
                        {
                            ...
                            {
                                string[] _ids = _params[2].Split(' ');
                                SpawnEntity(new Vector3(_x1, _y1, _z1), _radius, _ids);
                            }
                            else
                            {
                                string[] _ids = { _params[2] };
                                SpawnEntity(new Vector3(_x1, _y1, _z1), _radius, _ids);
                            }
                        }
                        else
                    ...
                    if (float.TryParse(_params[0], out _x1))
                    {
                        if (float.TryParse(_params[1], out _z1))
                        {
                            ...
                            if (float.TryParse(_params[2], out _radius))
                            {
                                ...
                                {
                                    string[] _ids = _params[3].Split(' ');
                                    SpawnEntity(new Vector3(_x1, _y1, _z1), _radius, _ids);
                                }
                                else
                                {
                                    string[] _ids = { _params[3] };
                                    SpawnEntity(new Vector3(_x1, _y1, _z1), _radius, _ids);
                                }
                            }
                            else
                        ...
                        if (float.TryParse(_params[1], out _y1))
                        {
                            if (float.TryParse(_params[2], out _z1))
                            {
                                if (float.TryParse(_params[3], out _radius))
                                {
                                    ...
                                    if (_params[4].Contains(" "))
                                    {
                                        string[] _ids = _params[4].Split(' ');
                                        SpawnEntity(new Vector3(_x1, _y1, _z1), _radius, _ids);
                                    }
                                    else
                                    {
                                        string[] _ids = { _params[4] };
                                        SpawnEntity(new Vector3(_x1, _y1, _z1), _radius, _ids);
                                    }
                                }
                            }
        ...
        private static void SpawnEntity(Vector3 _pos, float _radius, string[] _ids)
        {
            try
            {
                int _x2, _y2, _z2;
                ...
                {
                    if (int.TryParse(_ids[i], out int _id))
                    {
                        PersistentOperations.EntityId.TryGetValue(_id, out int _entityId);
