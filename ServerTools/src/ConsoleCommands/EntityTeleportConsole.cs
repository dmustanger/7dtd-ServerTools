        ...
        public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
        {
            try
            {
                ...
                if (_params.Count == 2)
                {
                    ...
                    {
                        SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Invalid entity id value: {0}", _entityId1));
                        return;
                    }
                    if (!int.TryParse(_params[1], out int _entityId2))
                    {
                        SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Invalid entity id value: {0}", _entityId2));
                        return;
                    }
                    Entity Entity1 = GameManager.Instance.World.Entities.dict[_entityId1];
                    if (Entity1 == null)
                    {
                        SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Entity not found: {0}", _entityId1));
                        return;
                    }
                    ...
                    {
                        SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Entity with id {0} can not be a player. Use a different command for teleporting players", _entityId1));
                        return;
                    }
                    Entity1.SetPosition(Entity2.position);
                    SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Teleported entity {0} to entity {1} located at {2} {3} {4}", _entityId1, _entityId2, (int)Entity2.position.x, (int)Entity2.position.y, (int)Entity2.position.z));
                }
                if (_params.Count == 4)
                {
                    ...
                    {
                        SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Entity with id {0} can not be a player. Use a different command for teleporting players", _entityId));
                        return;
                    }
                    if (!int.TryParse(_params[1], out int _x))
                    {
                        SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Invalid entityId value: {0}", _x));
                        return;
                    }
                    if (!int.TryParse(_params[2], out int _y))
                    {
                        SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Invalid entityId value: {0}", _y));
                        return;
                    }
                    if (!int.TryParse(_params[3], out int _z))
                    {
                        SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Invalid entityId value: {0}", _z));
                        return;
                    }
                    Entity.SetPosition(new UnityEngine.Vector3(_x, _y, _z));
                    SingletonMonoBehaviour<SdtdConsole>.Instance.Output(string.Format("[SERVERTOOLS] Teleported entity {0} to {1} {2} {3}", _entityId, _x, _y, _z));
                }
            }
            catch (Exception e)
            {
