...
namespace ServerTools
{
    public class ResetPlayerConsole : ConsoleCmdAbstract
    {
        ...
        public static void Reset(string _id)
        {
            ...
            _result.Dispose();
            _sql = string.Format("SELECT steamid FROM Waypoints WHERE steamid = '{0}'", _id);
            ...
            _result.Dispose();
            _sql = string.Format("SELECT steamid FROM Tracking WHERE steamid = '{0}'", _id);
            _result = SQL.TQuery(_sql);
            if (_result.Rows.Count != 0)
            {
