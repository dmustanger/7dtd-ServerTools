            ...
            try
            {
                ...
                if (_params[0].ToLower().Equals("off"))
                {
                    if (PlayerChecks.GodEnabled)
                    {
                        ...
                        LoadConfig.WriteXml();
                        SdtdConsole.Instance.Output(string.Format("Godmode detector has been set to off"));
                        return;
                    }
                    else
                    {
                        SdtdConsole.Instance.Output(string.Format("Godmode detector is already off"));
                        return;
                    }
                }
                else if (_params[0].ToLower().Equals("on"))
                {
                    if (!PlayerChecks.GodEnabled)
                    {
                        ...
                        LoadConfig.WriteXml();
                        SdtdConsole.Instance.Output(string.Format("Godmode detector has been set to on"));
                        return;
                    }
                    else
                    {
                        SdtdConsole.Instance.Output(string.Format("Godmode detector is already on"));
                        return;
                    }
                }
                else
