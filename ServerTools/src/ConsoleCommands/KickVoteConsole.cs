        ...
        public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
        {
            try
            {
                ...
                if (_params[0].ToLower().Equals("off"))
                {
                    ...
                    {
                        SdtdConsole.Instance.Output(string.Format("[SERVERTOOLS] Kick vote is already off"));
                        return;
                    }
                }
                else if (_params[0].ToLower().Equals("on"))
                {
                    ...
                    {
                        SdtdConsole.Instance.Output(string.Format("[SERVERTOOLS] Kick vote is already on"));
                        return;
                    }
                }
                else
