...
namespace ServerTools
{
    public class DonorConsole : ConsoleCmdAbstract
    {
        ...
        {
            return "Usage:\n" +
                "  1. st-don add <steamId> <name> <group> <prefix> <nameColor> <prefixColor> <days to expire>\n" +
            ...
            try
            {
                ...
                else if (_params[0].ToLower().Equals("add"))
                {
                    ...
                    if (_cInfo == null)
                    {
                        ...
                        else if (_params.Count == 8)
                        {
                            foreach (var group in ChatColorPrefix.Dict)
                            {
                                if (group.Value[1] == _params[3])
                                {
                                    SdtdConsole.Instance.Output(string.Format("[SERVERTOOLS] Can not add id {0} to the chat color prefix list. Group {1} already exists on the list. Create a new entry or add them to this group", _params[1], _params[3]));
                                    return;
                                }
                            }
                            ...
                            if (!double.TryParse(_params[7], out double _daysToExpire))
                            {
                                SdtdConsole.Instance.Output(string.Format("[SERVERTOOLS] Invalid days to expire: {0}", _params[7]));
                                return;
                            }
                            ...
                            if (ChatColorPrefix.Dict.ContainsKey(_params[1]))
                            {
                                ...
                                SdtdConsole.Instance.Output(string.Format("[SERVERTOOLS] Updated id {0}, with the name of {1} to the group {2} using prefix {3}, name color {4} and prefix color {5} that expires {6} to the chat color prefix list.", _params[1], _params[2], _params[3], _params[4], _params[5], _params[6], _expireDate.ToString()));
                            }
                            else
                            {
                                ChatColorPrefix.Dict.Add(_params[1], _c);
                                ChatColorPrefix.Dict1.Add(_params[1], _expireDate);
                                SdtdConsole.Instance.Output(string.Format("[SERVERTOOLS] Added id {0}, with the name of {1} to the group {2} using prefix {3}, name color {4} and prefix color {5} that expires {6} to the chat color prefix list.", _params[1], _params[2], _params[3], _params[4], _params[5], _params[6], _expireDate.ToString()));
                            }
