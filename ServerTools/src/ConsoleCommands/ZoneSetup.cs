...
{
    class ZoneSetup : ConsoleCmdAbstract
    {
        public static Dictionary<int, int> newZone = new Dictionary<int, int>();

        public override string GetDescription()
        {
            return "[ServerTools]-Setup a new zone or check existing zones.";
        }
        ...
        public override string GetHelp()
        {
            return "Usage:\n" +
                   "  1. zone new\n" +
                   "  2. zone list\n" +
                   "  3. zone delete <number>\n" +
                   "  4. zone save\n" +
                   "  5. zone back\n" +
                   "1. Create a new zone\n" +
                   "2. Shows the zone list\n" +
                   "3. Deletes the specific zone from the list\n" +
                   "4. Use this throughout the setup process when prompted\n" +
                   "5. Use this to go back a step in the setup process\n";
        }

        public override string[] GetCommands()
        {
            return new string[] { "st-zone", "zone" };
        }
        ...
        public override void Execute(List<string> _params, CommandSenderInfo _senderInfo)
        {
            try
            {
                ...
                if (_cInfo != null)
                {
                    ...
                    if (_params[0].ToLower().Equals("new"))
                    {
            ...
            {
                Log.Out(string.Format("[SERVERTOOLS] Error in ZoneSetup.Run: {0}.", e));
            }
        }
    }
}
